...
class JutsuClassifier():
    def __init__(self,
        ...
        if not huggingface_hub.repo_exists(self.model_path):
            ...
            # checking if the data path is provided...
    ...
    def load_model(self, model_path):
        try:
            model = pipeline('text-classification', model=model_path, device=self.device)
            
            if hasattr(self, 'label_dict'):
                model.model.config.id2label = self.label_dict
                model.model.config.label2id = {v: k for k, v in self.label_dict.items()}

            if not hasattr(self, 'label_dict'):
                    self.label_dict = {0: "Genjutsu", 1: "Ninjutsu", 2: "Taijutsu"}
                
        except Exception as e:
            print(f"Model on Hub is invalid ({e}). Loading base model...")
            model = pipeline('text-classification', model="distilbert-base-uncased", device=self.device)
    ...
    def postprocess(self, model_output):
        ...
        for pred in model_output:
            ...
            if label.startswith("LABEL_"):
                ...
                label = self.label_dict.get(index, label)
